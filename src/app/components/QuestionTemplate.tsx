...
const QuestionTemplate: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleNumQuestionsChange = (type: string, value: string) => {
    if (/^[0-9]*$/.test(value)) { // Allow only positive numbers
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-4/5 h-4/5 overflow-auto relative"> {/* Fill 80% of the screen */}
          ...
          <div key={index} className="mb-6 relative">
            ...
            <div className="relative border rounded px-2 py-1 w-full focus-within:ring-2 focus-within:ring-blue-500">
              ...
              <label 
                ...
                ${question ? 'text-blue-500 text-xs -top-2 bg-white px-1' : 'text-gray-500 top-2'}`} // Adjusted label position
              >
          ...
          <div className="flex items-center justify-center border-dashed border-2 border-gray-300 rounded-md py-8">
            ...
            <div className="text-center">
              <svg className="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                <path d="M12 16v-4a4 4 0 0 1 4-4h16a4 4 0 0 1 4 4v4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M8 24h32M12 28h8m-8 4h8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <div className="mt-2 text-sm text-gray-600">
                <p>Drag & drop a file here, or <span className="text-blue-500 underline cursor-pointer">choose file</span></p>
                {uploadedFileName && <p className="mt-2 text-gray-700">Uploaded File: <strong>{uploadedFileName}</strong></p>} {/* Show uploaded file name */}
                {error && <p className="mt-2 text-red-500">{error}</p>} {/* Show error message */}
              </div>
            </div>
          </div>
        </div>
        ...
        {['Factual Recall', 'Conceptual Understanding', 'Analysis & Evaluation', 'Application & Problem-Solving', 'Open-Ended Discussion'].map((type, idx) => (
          <div key={idx} className="mb-6 relative">
            <div className="relative border rounded px-2 py-1 w-full focus-within:ring-2 focus-within:ring-blue-500">
              <input
                type="number"
                value={numQuestions[`type${idx + 1}`]}
                onChange={(e) => handleNumQuestionsChange(`type${idx + 1}`, e.target.value)}
                placeholder=" " // Placeholder for the floating label effect
                className="w-full focus:outline-none bg-transparent pt-2 pb-1" // Adjusted padding for height
                min="1" // Allow only positive numbers
                style={{ zIndex: 1 }} // Ensure the input is on top
              ...
              <label 
                className={`absolute left-2 transition-all duration-200 ease-in-out pointer-events-none
                ${numQuestions[`type${idx + 1}`] ? 'text-blue-500 text-xs -top-2 bg-white px-1' : 'text-gray-500 top-2'}`} // Adjusted label position
              >
      ...
      <div className="flex justify-center w-full absolute bottom-4"> {/* Center the button */}
        <button 
          onClick={onClose} 
